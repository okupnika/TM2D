...
1022: add back feature in token out transformer
1022: add extra eval function in transformerV1.
1023: add extra eval function in transformerV2.
"""
...
class Decoder(nn.Module):
    ...
    def forward(self, trg_seq, trg_mask, enc_output, src_mask, return_attns=False):
        ...
        # debug_trg_seq_max = torch.max(trg_seq * 1.)
        # print('trg_seq.shape: ', trg_seq.shape)
        # print('debug_trg_seq_max: ', debug_trg_seq_max)
...
class TransformerV1(nn.Module):
    ...
    def encoding(self, src_seq, src_non_pad_lens):
        batch_size, src_seq_len = src_seq.shape[0], src_seq.shape[1]
        src_mask = get_pad_mask(batch_size, src_seq_len, src_non_pad_lens).to(src_seq.device)
        enc_output, *_ = self.encoder(src_seq, src_mask)
        return enc_output, src_mask

    def decoding(self, trg_seq, enc_output, src_mask):
        trg_mask = get_pad_mask_idx(trg_seq, self.trg_pad_idx) & get_subsequent_mask(trg_seq)
        dec_output, *_ = self.decoder(trg_seq, trg_mask, enc_output, src_mask)
        seq_logit = self.trg_word_prj(dec_output)
        return seq_logit
    ...
    def sample_with_enc_output(self, src_seq, src_non_pad_lens, enc_output, trg_sos, trg_eos, start_idx=0, end_idx=80, sample=False, top_k=None):
        trg_seq = torch.LongTensor(src_seq.size(0), 1).fill_(trg_sos).to(src_seq).long()
        ...
        for i, _ in enumerate(range(start_idx, end_idx)):
            # print(trg_seq)
            trg_mask = get_subsequent_mask(trg_seq)
            dec_output, *_ = self.decoder(trg_seq, trg_mask, enc_output, src_mask)
            seq_logit = self.trg_word_prj(dec_output)
    ...
    def sample_with_double_enc_output(self, src_seq, enc_output_a, src_mask_a, weight_a, enc_output_b, src_mask_b,
                                      trg_sos, trg_eos, start_idx=0, end_idx=80, sample=False, top_k=None):
        trg_seq = torch.LongTensor(src_seq.size(0), 1).fill_(trg_sos).to(src_seq).long()

        for i in range(start_idx, end_idx):
            # print(trg_seq)
            trg_mask = get_subsequent_mask(trg_seq)
            dec_output_a, *_ = self.decoder(trg_seq, trg_mask, enc_output_a, src_mask_a)

            dec_output_b, *_ = self.decoder(trg_seq, trg_mask, enc_output_b, src_mask_b)

            # 1001: 尝试在regression前面做融合看看.
            # 加权融合:  Bxtxc
            dec_output_mix = weight_a[i] * dec_output_a[:, -1, :] + (1 - weight_a[i]) * dec_output_b[:, -1, :]
            seq_logit_mix = self.trg_word_prj(dec_output_mix)
            logits_mix = seq_logit_mix
            if top_k is not None:
                logits_mix = top_k_logits(logits_mix, top_k)
            probs_mix = F.softmax(logits_mix, dim=-1)
    ...
    def sample_with_trg_seq_enc_output(self, src_seq, src_non_pad_lens, enc_output, trg_sos, trg_eos, trg_seq, start_idx=0, end_idx=80, sample=False, top_k=None):
        # trg_seq = torch.LongTensor(src_seq.size(0), 1).fill_(trg_sos).to(src_seq).long()
    ...
    def sample_with_trg_seq_double_enc_output(self, src_seq, enc_output_a, src_mask_a, weight_a, enc_output_b, src_mask_b,
                                              trg_sos, trg_eos, trg_seq, start_idx=0, end_idx=80, sample=False, top_k=None):
        # trg_seq = torch.LongTensor(src_seq.size(0), 1).fill_(trg_sos).to(src_seq).long()
        ...
        for i, _ in enumerate(range(start_idx, end_idx)):
            ...
            # 1001: try fusion before regression:  Bxtxc
            dec_output_mix = weight_a[i] * dec_output_a[:, -1, :] + (1 - weight_a[i]) * dec_output_b[:, -1, :]
            seq_logit_mix = self.trg_word_prj(dec_output_mix)
            logits_mix = seq_logit_mix
            if top_k is not None:
...
class TransformerV2(nn.Module):
    ...
    def encoding(self, src_seq, input_onehot=False, src_mask=None, src_non_pad_lens=None):
        batch_size, src_seq_len = src_seq.shape[0], src_seq.shape[1]
        # src_mask = get_pad_mask(batch_size, src_seq_len, src_non_pad_lens).to(src_seq.device)
        if not input_onehot:
            src_mask = get_pad_mask_idx(src_seq, self.src_pad_idx)
    ...
    def sample_with_enc_output(self, src_seq, enc_output, trg_sos, trg_eos, start_idx=0, end_idx=80, sample=False, top_k=None):
        trg_seq = torch.LongTensor(src_seq.size(0), 1).fill_(trg_sos).to(src_seq).long()
    ...
    def sample_with_trg_seq_enc_output(self, src_seq, enc_output, trg_sos, trg_eos, trg_seq, start_idx=0, end_idx=80, sample=False, top_k=None):
        # trg_seq = torch.LongTensor(src_seq.size(0), 1).fill_(trg_sos).to(src_seq).long()
...
"""Of which the combination of t2m_tranformer and m2t_tranformer"""
class TransformerX(nn.Module):
    def __init__(self, m2t_transformer, t2m_transformer):
...
class TransformerX1(nn.Module):
    def __init__(self, d2a_transformer, a2d_transformer):
        super(TransformerX1, self).__init__()
        self.d2a_transformer = d2a_transformer
        self.a2d_transformer = a2d_transformer

class TransformerX2(nn.Module):
    def __init__(self, d2a_transformer, a2d_transformer, m2t_transformer, t2m_transformer):
        super(TransformerX2, self).__init__()
        self.d2a_transformer = d2a_transformer
        self.a2d_transformer = a2d_transformer
        self.m2t_transformer = m2t_transformer
        self.t2m_transformer = t2m_transformer

class TransformerX3(nn.Module):
    def __init__(self, a2d_transformer, t2m_transformer, m2m_transformer):
        super(TransformerX3, self).__init__()
        self.a2d_transformer = a2d_transformer
        self.t2m_transformer = t2m_transformer
        self.m2m_transformer = m2m_transformer
